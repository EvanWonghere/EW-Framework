...
- 已实现（Core）：`ObjectPool`、`SharedVariables`、`Singleton`、`SOEventBus`、`StateMachine`、`TimerSystem`（均含模块文档与 Examples）
...
- **ObjectPool**：基于 Unity `IObjectPool<T>` 的对象池（同步 GameObject 池 / Addressables 异步池 / 纯 C# 引用池）。
  ...
  - 文档：`Assets/EW_Framework/Core/ObjectPool/README.md`；示例：`Assets/EW_Framework/Core/ObjectPool/Examples/README.md`
- **SharedVariables**：基于 ScriptableObject 的共享变量（变更事件 + 可选持久化 `ISaveable`）。
  ...
  - 文档：`Assets/EW_Framework/Core/SharedVariables/README.md`；示例：`Assets/EW_Framework/Core/SharedVariables/Examples/README.md`
- **Singleton**：单例基类集合（纯 C# `Singleton<T>`、场景级 `MonoSingleton<T>`、跨场景 `PersistentMonoSingleton<T>`）。
  - 统一懒加载与防御逻辑，便于 Core/Modules 的管理器实现。
  - 文档：`Assets/EW_Framework/Core/Singleton/README.md`
- **SOEventBus**：ScriptableObject Event Channel（原子通道 & 聚合通道 `SafeEvent`）。
  ...
  - 文档：`Assets/EW_Framework/Core/SOEventBus/README.md`；示例：`Assets/EW_Framework/Core/SOEventBus/Examples/README.md`
- **StateMachine**：轻量泛型 FSM（按状态类型缓存实例，支持 ChangeState 与 Push/Pop）。
  - Context 驱动状态逻辑；`IStackState<T>` 提供 Pause/Resume 语义。
  - 文档：`Assets/EW_Framework/Core/StateMachine/README.md`；示例：`Assets/EW_Framework/Core/StateMachine/Examples/README.md`
- **TimerSystem**：全局计时器管理器 + 可池化 Timer 句柄（Delay/Loop、Pause/Resume/Cancel、Scaled/Unscaled）。
  ...
  - 文档：`Assets/EW_Framework/Core/TimerSystem/README.md`；示例：`Assets/EW_Framework/Core/TimerSystem/Examples/README.md`
...
- `Core/` 下的 `Examples/` 仅用于验证子模块 API 与最佳实践；正式 Demo 场景建议集中放到 `Assets/EW_Framework/Demo/`，以满足依赖方向与叙事组织。
...
3. 快速跑通 Core Examples（可直接打开并 Play）：
...
A: Core 模块的 Examples 场景已可运行（见「快速开始」第 3 步）。Gameplay Demo 场景将按 `Docs/Planning/EWF_DEMO_DESIGN.md` 推进落地后逐步可运行。
