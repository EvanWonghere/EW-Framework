    ...
    public class AsyncPoolManager : PersistentMonoSingleton<AsyncPoolManager>
    {
        ...
        private async UniTask<AsyncPoolData> CreateAsyncPool(AssetReference assetRef, string guid)
        {
            try
            {
                ...
                // Await completion then read Result — avoids CS0029 when the compiler picks a void ToUniTask overload.
                await handle.ToUniTask(cancellationToken: this.GetCancellationTokenOnDestroy());
                GameObject loadedPrefab = handle.Status == AsyncOperationStatus.Succeeded ? handle.Result : null;
